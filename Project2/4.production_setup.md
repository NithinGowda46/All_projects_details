...
This section covers the production infrastructure.
...
### 2.6 Allow HTTPS in ALB Security Group

Allow HTTPS traffic on port `443` in the **EKS ALB Security Group**.
...
<img src="images/alb-security-group-https.png" alt="ALB Security Group HTTPS" width="350">
...
> **Note:** In the current configuration, CloudFront connects to the ALB using **HTTP :80**, so port `443` is not currently used for the CloudFront-to-ALB connection. The HTTPS `443` rule is kept in the ALB Security Group for the production HTTPS configuration and becomes required if HTTPS is later enabled between CloudFront and the ALB.
...
| ALB security group | HTTPS :443 allowed |
